...
struct SolverTiming
{
	...
	void OnGUI()
	{
		...
		if (!hasPrinted && Timer::physicsFrameCount() == printAtFrame)
		{
			...
			//std::cout << "Info(GUI): Average solver time at frame(" << printAtFrame << ") is: " << label2avgTime["KernelSum"] / count << " ms\n";
			std::cout << "Info(GUI): Average solver time at frame(" << printAtFrame << ") is: " << std::fixed << std::setprecision(3) << label2avgTime["KernelSum"] / count << " ms" << std::endl;
		}
...
struct PerformanceStat
{
	...
	void OnGUI()
	{
		...
		auto overlay = std::string("Solver: ") + std::to_string(solverTime) + " ms (" + std::to_string(solverTime > 0 ? (1000.0 / solverTime) : 0) + " FPS)";
